...
-- "Sailor_BarrierExpirationTurn" (Game)
-- 
-- Context: UI
-- "Sailor_ValkranaObtained" .. greatWork (Game)
...
function Sailor_Valkrana_Reanimation(tUnitData, ownerID, valkID, yahooID)
    ...
    local iskeleRoll = TerrainBuilder.GetRandomNumber(100, "Skeleton Reanimation, Attacker") + 1;
...
function Sailor_ValkranaTargeter(leaderID, valkranaID, spellmode)
    ...
    if cityNum > 0 then
        ...
        if spellmode == 1 then
            ...
            repeat
                local iRoll = TerrainBuilder.GetRandomNumber(cityNum, "Target Roller") + 1
        ...
        local iRoll2 = TerrainBuilder.GetRandomNumber(cityNum, "Target Roller 2") + 1
...
function Sailor_ValkranaTargeterRand(valkranaID, spellmode)
    local iRoll = TerrainBuilder.GetRandomNumber(#tLandPlots, "Plot Roller") + 1
...
function Sailor_Valkrana_Spell4(valkranaID, iX, iY)
    ...
    local i = 0;
    while i < skeleNum do
        local plotRoll = TerrainBuilder.GetRandomNumber(#tPlots, "Skeleport Plot Roller");
...
function Sailor_ValkranaFreeSkeletons(playerID, defeatType, eventID)
	...
	-- Variable spawn depending on difficulty.
    local spawnNum  = 3 + (gameDifficultyIndex * 2);
    local i         = 0;
    while i < spawnNum do
        local iRoll     = TerrainBuilder.GetRandomNumber(#tLandPlots, "Plot Roller") + 1
        local plot;
        ...
        if not plot:IsCity() and not plot:IsImpassable() then
            UnitManager.InitUnit(63, valkUnitName, plot:GetX(), plot:GetY());
            i = i + 1;
        end
    end
end
...
function Sailor_Valkrana_SetProp(proptype, propunique, value, playerID)
    ...
    if playerID == nil then
        ...
        Players[playerID]:SetProperty(proptype, value);
		return;
    end
end
...
function Initialize()
    for k, v in ipairs(PlayerManager.GetWasEverAliveIDs()) do
        ...
        if sLeaderType == 'LEADER_SAILOR_VALKRANA' then
            print("///// Valkrana detected. Loading functions...");
            print("///// Necromancers are never short on friends. They have them in spades.");
            ...
            if not Players[v]:IsHuman() then
                print("///// Valkana AI detected.");
                Events.UnitAddedToMap.Add(Sailor_ValkranaAI);       -- AI stuff
            end
            return;
        end
    end
    print("///// Valkrana not detected.");
end
Initialize();