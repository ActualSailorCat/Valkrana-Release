...
-- Relics
INSERT INTO Types (Type, Kind) SELECT 'BUILDING_SAILOR_VALKRANA_'||RowID, 'KIND_BUILDING' FROM Routes;
INSERT INTO Buildings (BuildingType, Name, Cost, PrereqDistrict, InternalOnly) SELECT 'BUILDING_SAILOR_VALKRANA_'||RowID, 'Placeholder', 0, 'DISTRICT_CITY_CENTER', 1 FROM Routes;
INSERT INTO TraitModifiers (TraitType, ModifierId) SELECT 'TRAIT_AGENDA_SAILOR_VALKRANA', 'AGENDA_SAILOR_VALKRANA_RELIC_'||RowID FROM Routes;
...
INSERT INTO Modifiers
		(ModifierId,							ModifierType,									SubjectRequirementSetId)
SELECT	'AGENDA_SAILOR_VALKRANA_RELIC_'||RowID,	'MODIFIER_PLAYER_DIPLOMACY_SIMPLE_MODIFIER',	'SAILOR_VALKRANA_RELIC_REQUIREMENTSET_'||RowID FROM Routes;
...
SELECT 'AGENDA_SAILOR_VALKRANA_RELIC_'||RowID, 'InitialValue', 3 FROM Routes;
...
*/
INSERT INTO ModifierArguments (ModifierId, Name, Value)
SELECT 'AGENDA_SAILOR_VALKRANA_RELIC_'||RowID, 'StatementKey', 'LOC_DIPLO_KUDO_LEADER_SAILOR_VALKRANA_REASON_RELIC' FROM Routes;

INSERT INTO ModifierArguments (ModifierId, Name, Value)
SELECT 'AGENDA_SAILOR_VALKRANA_RELIC_'||RowID, 'SimpleModifierDescription', 'LOC_DIPLO_MODIFIER_SAILOR_VALKRANA_RELIC' FROM Routes;
...
SELECT 'AGENDA_SAILOR_VALKRANA_RELIC_'||RowID, 'Sample', 'LOC_TOOLTIP_SAMPLE_DIPLOMACY_ALL' FROM Routes;
...
SELECT 'SAILOR_VALKRANA_RELIC_REQUIREMENTSET_'||RowID, 'REQUIREMENTSET_TEST_ALL' FROM Routes;
...
SELECT 'SAILOR_VALKRANA_RELIC_REQUIREMENTSET_'||RowID, 'SAILOR_VALKRANA_REQUIRES_'||RowID||'_RELICS' FROM Routes;
...
SELECT	'SAILOR_VALKRANA_REQUIRES_'||RowID||'_RELICS', 'REQUIREMENT_PLAYER_HAS_BUILDING', 1 FROM Routes;
...
SELECT	'SAILOR_VALKRANA_REQUIRES_'||RowID||'_RELICS', 'BuildingType', 'BUILDING_SAILOR_VALKRANA_'||RowID FROM Routes;
--///////////////////////////////////////////////////////
-- Valkrana / AI
--///////////////////////////////////////////////////////
...
-- PSEUDOYIELDS
		...
		('SAILOR_VALKRANA_PseudoYields',	1,			'PSEUDOYIELD_GREATWORK_RELIC',		150),
		...
		('SAILOR_VALKRANA_PseudoYields',	1,			'PSEUDOYIELD_UNIT_SPY',				50),
...
-- BUILDINGS & DISTRICTS			
		...
		('SAILOR_VALKRANA_Buildings',		1,			'BUILDING_GREAT_LIBRARY',			25),
		('SAILOR_VALKRANA_Buildings',		1,			'BUILDING_MONT_ST_MICHEL',			25),
		('SAILOR_VALKRANA_Buildings',		1,			'BUILDING_OXFORD_UNIVERSITY',		0),
-- CIVICS & TECHS
		('SAILOR_VALKRANA_Civics',			1,			'CIVIC_DIPLOMATIC_SERVICE',			50), 
		('SAILOR_VALKRANA_Civics',			1,			'CIVIC_NATIONALISM',				0), 
		('SAILOR_VALKRANA_Civics',			1,			'CIVIC_COLD_WAR',					0), 
		('SAILOR_VALKRANA_Civics',			1,			'CIVIC_IDEOLOGY',					0),
		('SAILOR_VALKRANA_Techs',			1,			'TECH_COMPUTERS',					0),  
		('SAILOR_VALKRANA_Techs',			1,			'TECH_CHEMISTRY',					50),
		('SAILOR_VALKRANA_Techs',			1,			'TECH_EDUCATION',					50),
...
-- BUILDINGS
INSERT INTO AiFavoredItems	(ListType, Favored, Item, Value) SELECT 'SAILOR_VALKRANA_Buildings', 1, BuildingType, 0    
FROM Buildings WHERE PrereqDistrict = 'DISTRICT_CAMPUS';
--INSERT INTO AiFavoredItems (ListType, Favored, Item, Value) SELECT 'SAILOR_VALKRANA_Buildings', 1, BuildingType, 0 FROM Building_GreatWorks WHERE GreatWorkSlotType = 'GREATWORKSLOT_RELIC';
-- // Expansion and DLC stuff here.
INSERT INTO AiFavoredItems (ListType, Favored, Item, Value) SELECT 'SAILOR_VALKRANA_Buildings', 1, 'BUILDING_APADANA', 0
WHERE EXISTS (SELECT BuildingType FROM Buildings WHERE BuildingType = 'BUILDING_APADANA');
INSERT INTO AiFavoredItems (ListType, Favored, Item, Value) SELECT 'SAILOR_VALKRANA_Buildings', 1, 'BUILDING_GOV_SPIES', 0
WHERE EXISTS (SELECT BuildingType FROM Buildings WHERE BuildingType = 'BUILDING_GOV_SPIES');
INSERT INTO AiFavoredItems (ListType, Favored, Item, Value) SELECT 'SAILOR_VALKRANA_Buildings', 1, 'BUILDING_ST_BASILS_CATHEDRAL', 25
WHERE EXISTS (SELECT BuildingType FROM Buildings WHERE BuildingType = 'BUILDING_ST_BASILS_CATHEDRAL');
INSERT INTO AiFavoredItems (ListType, Favored, Item, Value) SELECT 'SAILOR_VALKRANA_Buildings', 1, 'BUILDING_UNIVERSITY_SANKORE', 0
